...
<div class="">
    <div class="row">
        <div id="info-container" class="">
            <h1>{{ $contact->cName }} {{ $contact->cLastName }}</h1>
            <h2>Telefone: {{ $contact->cPhone }}</h2>
            <h2>E-mail: {{ $contact->cEmail }}</h2>
        </div>
    </div>
</div>
<a href="/contacts/edit/{{$contact->id}}" class="btn third">Editar</a>
<form action="/contacts/{{$contact->id}}" method="POST">
    @method('DELETE')
    <button type="submit" class="btn btndanger delete-btn" name="thash-outline">Deletar</button>
</form>
